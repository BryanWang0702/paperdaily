...
class TestLocalBundle(unittest.TestCase):
    def test_bundle_contains_launchers_but_never_private_state(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("PaperDaily-local/VERSION", names)
            self.assertIn("PaperDaily-local/standalone_version.json", names)
            ...
            self.assertFalse(any(name.endswith("local_status.json") for name in names))
